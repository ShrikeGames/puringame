...
var reference_ai_name: String = "best"
...
var purin_sorter: PurinSorter
var time_since_revaluated_purin: float = 0
var AI_REVALUATE_PURIN_DELAY: float = 0.5
...
func get_valid_drop_position():
	...
	var spawn_x_pos: float = min(
		max(bowl.position.x + BOWL_THICKNESS, noir_pos.x),
		bowl.position.x + BOWL_WIDTH - BOWL_THICKNESS
	)
	...
	if ai_controlled:
		...
		ai_adjusted_score += (
			...
			* max(
				...
				(
					score_increase
					- (purin_node.get_child_count() * AI_PURIN_COUNT_PENALTY_WEIGHT)
					- (dropped_purin_count * AI_PURIN_COUNT_PENALTY_WEIGHT)
				)
			)
		)
...
func _process(delta):
	...
	if has_purin_in_danger:
		if game_over_timer >= game_over_grace_period_seconds:
			print(
				"%s Game Overed with a score of %s (%s)" % [player_name, score, ai_adjusted_score]
			)
			if score > highscore:
				highscore = score
	...
	if time_since_keypress >= using_mouse_time_threshold:
		...
		if (
			mouse_pos.x >= bowl.position.x + self.position.x
			and mouse_pos.x <= bowl.position.x + BOWL_WIDTH + self.position.x
		):
			use_mouse_controls = true
...
func check_to_drop_purin(_delta):
	...
	if Input.is_action_just_pressed("drop_purin"):
		if use_mouse_controls:
			...
			if (
				mouse_pos.x <= bowl.position.x + self.position.x
				or mouse_pos.x >= bowl.position.x + BOWL_WIDTH + self.position.x
			):
				return
...
func set_up_ai():
	...
	if mutant_type < ai_use_best_rate:
		...
		AI_X_WEIGHT = get_and_mutate(reference_ai_name, "x", -30.0, 30.0, -100.0, 100.0)
		AI_Y_WEIGHT = get_and_mutate(reference_ai_name, "y", -30.0, 30.0, -100.0, 100.0)
		AI_LEVEL_WEIGHT = get_and_mutate(reference_ai_name, "l", -30.0, 30.0, -100.0, 100.0)
		AI_STACKED_WEIGHT = get_and_mutate(reference_ai_name, "s", -30.0, 30.0, 0.0, 100.0)
		AI_EASY_COMBINE_WEIGHT = get_and_mutate(reference_ai_name, "c", -30.0, 30.0, 0.0, 100.0)
		AI_DROP_DELAY_SECONDS = get_and_mutate(reference_ai_name, "d1", -0.5, 0.5, 0.5, 2.0)
		AI_DROP_DELAY_PANIC_SECONDS = get_and_mutate(
			reference_ai_name, "d2", -1.0, 1.0, AI_DROP_DELAY_SECONDS, AI_DROP_DELAY_SECONDS
		)
		drop_bias = get_and_mutate(reference_ai_name, "drop_bias", -20.0, 20.0, -50.0, 50.0)
		next_purin_weight = get_and_mutate(reference_ai_name, "np", -20.0, 20.0, 0.0, 50.0)
		...
		AI_DROP_DELAY_PANIC_SECONDS = get_and_mutate(
			player_name, "d2", -1.0, 1.0, AI_DROP_DELAY_SECONDS, AI_DROP_DELAY_SECONDS
		)
	...
	if debug:
		debug_text.text = (
			"X: %s, Y: %s, L:%s, S:%s, D1:%s, D2:%s, B:%s, np: %s, c:%s, Highscore:%s (%s)"
			% [
				round(AI_X_WEIGHT),
				round(AI_Y_WEIGHT),
				round(AI_LEVEL_WEIGHT),
				round(AI_STACKED_WEIGHT),
				AI_DROP_DELAY_SECONDS,
				AI_DROP_DELAY_PANIC_SECONDS,
				round(drop_bias),
				round(next_purin_weight),
				round(AI_EASY_COMBINE_WEIGHT),
				highscore,
				ai_adjusted_highscore
			]
		)

	time_since_ai_dropped_purin = 0

	purin_sorter = PurinSorter.new()
	purin_sorter.init(
		AI_X_WEIGHT,
		AI_Y_WEIGHT,
		AI_LEVEL_WEIGHT,
...
class PurinSorter:
	...
	func init(xw, yw, lw, sw, npw, db, c, debug_mode, purin_sizes):
		self.x_weight = xw
		self.y_weight = yw
	...
	func score_purin(
		...
		if num_above < num_above2:
			above_diff = -stacked_weight * num_above * 10
		var height_penalty: float = 0.0
		if y - (purin_sizes[level] * 0.5) * 1.17 < 138:
			# if it's an easy combine increase its liklihood
			if easy_combine > 0:
				easy_combine = 2 * easy_combine
			else:
				#otherwise penalize heavily putting it up so high
				height_penalty = 1000
		var combines_bonus = 1
		if possible_combines > possible_combines2 and num_above < num_above2:
			combines_bonus = possible_combines

		return (
			combines_bonus
			* (xdiff + ydiff + leveldiff + above_diff + easy_combine - height_penalty)
		)

	func prioritize_purin(
		purin1: Purin, purin2: Purin, held_purin_level: int, next_purin_level: int
	):
		if purin2 == null or purin2.linear_velocity.y > 1:
			return 1
			
		if purin1 == null or purin1.linear_velocity.y > 1:
			return 1
		
		var next_purin_score = 0
		if level_weight != 0:
			next_purin_score = score_purin(
				next_purin_level,
				purin1.position.x,
				purin1.position.y,
				purin1.get_meta("level"),
				purin1.number_possible_combines(),
				purin1.num_above_purin(),
				purin2.position.x,
				purin2.position.y,
				purin2.get_meta("level"),
				purin2.number_possible_combines(),
				purin2.num_above_purin()
			)
		var this_score = score_purin(
			held_purin_level,
			purin1.position.x,
			purin1.position.y,
			purin1.get_meta("level"),
	...
	func get_best_spawn_x(existing_purin, held_purin_level, next_purin_level):
		...
		for purin1 in existing_purin:
			var purin_score = prioritize_purin(
				purin1, best_purin, held_purin_level, next_purin_level
			)
			...
			if debug:
				...
				if purin_score < 0:
					text_colour = "FF0000"
				purin1.debug_text.text = (
					"[center][color=#%s]%s %s[/color][/center]"
					% [text_colour, purin1.get_meta("level"), rounded_score]
				)
		...
		if debug:
			...
			best_purin.debug_text.text = (
				"[center][color=#%s]%s %s[/color][/center]"
				% [best_text_colour, best_purin.get_meta("level"), best_score]
			)
...
func process_ai_turn(delta):
	...
	time_since_revaluated_purin += delta
	var max_wait: float = AI_DROP_DELAY_SECONDS
	if has_purin_in_danger:
		max_wait = (
			AI_DROP_DELAY_PANIC_SECONDS
			* ((game_over_grace_period_seconds - game_over_timer) / game_over_grace_period_seconds)
		)
	...
	var spawn_x = 0
	if (
		time_since_revaluated_purin >= max(AI_REVALUATE_PURIN_DELAY, max_wait)
		and purin_node.get_child_count() > 0
	):
		var existing_purin: Array[Node] = purin_node.get_children()
		if not existing_purin.is_empty():
			spawn_x = (
				bowl.position.x
				+ purin_sorter.get_best_spawn_x(existing_purin, held_purin_level, next_purin_level)
				- 0.5
			)
			noir.position.x = spawn_x
			noir.position = update_noir_pos()
			time_since_revaluated_purin = 0
