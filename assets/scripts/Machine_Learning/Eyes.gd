...
func get_inputs_from_raycasts(held_purin_level: int, _next_purin_level: int, left_edge: Node2D, right_edge: Node2D) -> Array:
	...
	for ray in raycasts:
		...
		if is_instance_valid(ray):
			...
			var x_pos: float = clampf((_input.x_pos - left_edge.position.x) / (right_edge.position.x - left_edge.position.x), 0.0, 1.0)
			_input_array.append(x_pos)
			
			# info about purin level
			if _input.is_purin:
				...
				var level_match_score = 0
				if held_purin_level == _input.level:
					level_match_score = 1
				_input_array.append(level_match_score)
			else:
